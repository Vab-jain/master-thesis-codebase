"""PPO training with LLM hints.

Implements a minimal, single-env PPO loop with:
- Hint injection wrapper at cadence `k`
...
- Logging to CSV + plots, and fail-fast on excessive LLM failures
"""
...
from llm.client import GROQClient, LocalHFClient
...
class HintsConfig:
    ...
    local_model: str = "meta-llama/Meta-Llama-3-8B"
...
class TrainConfig:
    ...
    logging_dir: str = "runs/ppo_llm_scaffold"
...
def dict_to_train_config(cfg: Dict[str, Any]) -> TrainConfig:
    ...
    hints = HintsConfig(
        ...
        provider=str(hints_cfg.get("provider", "local")),
        groq_model=str(((hints_cfg.get("groq", {}) or {}).get("model", "llama-3.3-70b-versatile"))),
        local_model=str(((hints_cfg.get("local", {}) or {}).get("model", "llama3.1:latest"))),
    ...
    return TrainConfig(
        ...
        ppo=ppo,
        hints=hints,
        logging_dir=str(logging_cfg.get("output_dir", "runs/ppo_llm_scaffold")),
    )
...
def make_env(cfg: TrainConfig, use_hints: bool = True) -> HintInjectorEnv | NegotiationEnv:
    ...
    # Client
    if use_hints:
        provider = cfg.hints.provider.lower()
        client = None
        if provider == "groq":
            client = GROQClient(model=cfg.hints.groq_model)
        elif provider == "local_hf":
            client = LocalHFClient(model=cfg.hints.local_model)
        wrapped = HintInjectorEnv(env, client=client, k=cfg.hints.k, prompt_path=cfg.hints.prompt_path, provider=provider)
        return wrapped
    return env
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PPO + LLM-Hints Scaffold")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    while (len(returns_history) < episodes_target) and (steps_target is None or env_steps < steps_target):
        ...
        for step in range(cfg.ppo.rollout_steps):
            ...
            hint_ok = bool(info.get("hint_available", hint.get("hint_available", False)))
            # If using LLM provider and a hint call was attempted but failed, abort per constraints
            if (not args.no_hints) and str(cfg.hints.provider).lower() in ("groq", "local_hf"):
                if info.get("hint_called") and (not info.get("hint_call_success", False)):
                    raise RuntimeError("LLM hint failure: not selecting action for this step")
        ...
        # Precompute new outputs for the whole batch
        ...
        with llm_csv.open("a", newline="") as f:
            ...
            src = str(getattr(getattr(env, "_provider", ""), "lower", lambda: "")()) if hasattr(env, "_provider") else ""
            src = getattr(env, "_provider", src)
            w.writerow([update, total, failed, ratio, avg_lat, src])
        # Minimal stdout summary for LLM usage per update
        try:
        ...
        if total >= fp.min_total_calls and ratio > fp.stop_on_ratio_gt:
            ...
            with (out_dir / "status.json").open("w", encoding="utf-8") as f:
                json.dump({"status": "LLM_NOT_USED", "total_hint_calls": total, "failed_hint_calls": failed, "ratio": ratio}, f)
            return
    ...
    final = {
        **mets,
        ...
        "seed": int(run_seed),
        "algo": "ppo",
        "hint_mode": "llm" if not args.no_hints else "none",
        "k": int(cfg.hints.k),
