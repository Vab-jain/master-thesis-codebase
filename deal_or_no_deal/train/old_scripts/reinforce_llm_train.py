...
def make_env(max_turns: int, provider: str, groq_model: str, local_model: str, k: int, prompt_path: str):
    ...
    elif provider_l == "local_hf":
        client = LocalHFClient(model=local_model)
    return HintInjectorEnv(base, client=client, k=k, prompt_path=prompt_path, provider=provider_l)
...
def train(out_dir: str, total_episodes: int = 2000, seed: int = 1234, max_turns: int = 10,
          provider: str = "local_hf", groq_model: str = "llama-3.3-70b-versatile", local_model: str = "meta-llama/Meta-Llama-3-8B",
          k: int = 1, prompt_path: str = "configs/llm_prompt.txt", save_every_episodes: int = 100,
          total_env_steps: int = 0, entropy_coef: float = 0.01):
    os.makedirs(out_dir, exist_ok=True)
    ckpt_dir = Path(out_dir) / "checkpoints"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    np.random.seed(seed); torch.manual_seed(seed)
    env = make_env(max_turns, provider, groq_model, local_model, k, prompt_path)
    policy = PolicyNet()
    ...
    turns_used = []
    env_steps = 0
    for ep in trange(total_episodes, desc="REINFORCE_LLM"):
        obs, info = env.reset(seed=seed+ep)
        traj_logps = []; traj_rewards = []; steps=0; done=False
        while not done:
            # If using LLM providers, a failed hint call should abort instead of backing off to policy
            if info.get("hint_called") and (not info.get("hint_call_success", False)) and provider.lower() in ("groq", "local_hf"):
...
if __name__ == "__main__":
    ...
    p.add_argument("--provider", type=str, default="local_hf")
